...
RasterRgbRendererSample {
    ...
    function applyRendererSettings(){
        if (selectedType === minMax){
            applyMinMax(minMaxMin.itemAt(0).value, minMaxMin.itemAt(1).value, minMaxMin.itemAt(2).value,
                        minMaxMax.itemAt(0).value, minMaxMax.itemAt(1).value, minMaxMax.itemAt(2).value);
        }
        else if (selectedType === percentClip){
            applyPercentClip(percentClipMin.value, 100 - percentClipMax.value);
        }
        else if (selectedType === stdDeviation){
