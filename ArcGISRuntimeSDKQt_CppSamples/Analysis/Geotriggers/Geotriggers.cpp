...
{
  if (!m_currentFeaturesEntered["Section Trigger"].isEmpty())
    return m_currentFeaturesEntered["Section Trigger"].last();
  return "N/A";
}

QStringList Geotriggers::poisInRange() const
{
  return m_currentFeaturesEntered["POI Trigger"];
}
...
void Geotriggers::setMapView(MapQuickView* mapView)
{
  ...
  createGeotriggerMonitor(m_gardenSections, 0.0, "Section Trigger");
  createGeotriggerMonitor(m_gardenPois, 10.0, "POI Trigger");
}
...
void Geotriggers::createGeotriggerMonitor(ServiceFeatureTable* serviceFeatureTable, double bufferSize, const QString& geotriggerName)
{
  ...
  connect(geotriggerMonitor, &GeotriggerMonitor::geotriggerNotification, this, [this](GeotriggerNotificationInfo* geotriggerNotification)
  {
    handleGeotriggerNotification(geotriggerNotification);
  });
