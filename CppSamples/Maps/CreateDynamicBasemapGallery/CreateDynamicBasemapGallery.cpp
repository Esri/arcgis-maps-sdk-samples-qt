...
namespace
{
  QMap<QString, BasemapStyleLanguageStrategy> languageStrategyNameToEnumMap{{"Default", BasemapStyleLanguageStrategy::Default},
...
CreateDynamicBasemapGallery::CreateDynamicBasemapGallery(QObject* parent /* = nullptr */) :
  QObject(parent),
...
void CreateDynamicBasemapGallery::init()
{
  ...
  qmlRegisterType<CreateDynamicBasemapGallery>("Esri.Samples", 1, 0, "CreateDynamicBasemapGallerySample");
}
...
{
  const auto iteratorToInfoForSelectedStyle =
    std::find_if(m_styleInfos.begin(), m_styleInfos.end(), [nameOfSelectedStyle](const BasemapStyleInfo* info)
  {
...
void CreateDynamicBasemapGallery::loadBasemap(const QString& selectedStrategy, const QString& selectedLanguage, const QString& selectedWorldview)
{
  if (!m_selectedStyle)
  {
    return;
  ...
  if (!selectedLanguage.isEmpty() && selectedLanguage != "None")
  {
    ...
    const auto iteratorToLanguageInfoForSelectedLanguage =
      std::find_if(specificLanguages.begin(), specificLanguages.end(), [selectedLanguage](const BasemapStyleLanguageInfo* info)
    {
