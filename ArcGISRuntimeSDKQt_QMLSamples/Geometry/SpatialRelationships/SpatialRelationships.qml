...
Rectangle {
    ...
    MapView {
        ...
        GraphicsOverlay {
            ...
            Graphic {
                ...
                Component.onCompleted: {
                    let polyBuilder = ArcGISRuntimeEnvironment.createObject("PolygonBuilder", {spatialReference: SpatialReference.createWebMercator()});
            ...
            Graphic {
                ...
                Component.onCompleted: {
                    let polyBuilder = ArcGISRuntimeEnvironment.createObject("PolylineBuilder", {spatialReference: SpatialReference.createWebMercator()});
        ...
        onIdentifyGraphicsOverlayStatusChanged: {
            ...
            let identifiedGraphics = identifyGraphicsOverlayResult.graphics;
            if (identifiedGraphics.length < 1)
                return;

            // get the first identified graphic
            let graphic = identifiedGraphics[0];
            ...
            // get the geometry
            let selectedGeometry = graphic.geometry;
            let selectedGeometryType = selectedGeometry.geometryType;
            ...
            if (selectedGeometryType !== Enums.GeometryTypePoint) {
                let pointRelationships = getSpatialRelationships(selectedGeometry, pointGraphic.geometry).toString();
                pointText.text = "Point: %1".arg(pointRelationships);
            }
            if (selectedGeometryType !== Enums.GeometryTypePolyline) {
                let polylineRelationships = getSpatialRelationships(selectedGeometry, polylineGraphic.geometry).toString();
                lineText.text = "Polyline: %1".arg(polylineRelationships);
            }
            if (selectedGeometryType !== Enums.GeometryTypePolygon) {
                let polygonRelationships = getSpatialRelationships(selectedGeometry, polygonGraphic.geometry).toString();
